...
社員が登録した配送・設置案件を、現場担当者が月間スケジュールから確認できるシステムです。案件の時間帯だけでなく、依頼内容、住所、同行有無、集合場所、車両、作業メモなどを構造化して管理します。

Excelに近い30分単位の一覧性を残しつつ、各セルから詳細画面へ移動できる構成にしました。PCでの入力を主対象とし、iPhone Safariからも同じ情報を確認できます。
...
- 各案件の詳細を画面上で共有できるようになり、毎朝、口頭やメールで個別に確認する手間が減った。その結果、案件の伝達漏れなどのトラブルも少なくなった
...
- **クラウド向け入口保護:** 共通パスワードゲート、CSRF防御、試行制限、セッション失効、ログアウトを有効にし、秘密値は環境変数で管理する
...
初期MVPと初回導入機能、UI改善、クラウド対応まで完了しています。社員が架空データで主要操作を確認し、重大な問題は検出されませんでした。現在は5人規模の限定利用が許可されています。H2、PostgreSQL Testcontainers、Playwright Chromiumを含む自動テストをGitHub Actionsで実行しています。
...
このDocker構成は、単独利用のローカル確認とデモを目的とする。複数人で共有試験を行う場合は、READMEの「クラウド試験用設定」どおりPostgreSQL構成を別途用意する。
...
無料クラウド試験では `cloud` profileを使い、PostgreSQLと共通パスワードゲートを有効にする。これは利用者を識別するためのログイン機能ではなく、URLを知っている人だけが開ける状態に近づけるための入口制限である。利用者は共有URLを開いた後、画面上で共通パスワードだけを入力する。cloud profileでアクセスゲートを無効化した場合は起動を中止し、すべての状態変更POSTはCSRFトークンで保護する。同一送信元から15分以内に5回失敗すると15分間ログインを拒否し、ログイン後のセッションは30分の無操作で失効する。月間画面から明示的にログアウトできる。
...
- 5人の利用者が、現行の共有Excelと同様に1つのスケジュールを共同利用する
