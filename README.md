...
配送・設置の予定と作業に必要な情報を、職場の10人弱で共有するWebアプリです。PCとiPhoneから案件を登録・確認でき、現在は実務で運用しています。

## 概要

開発者自身が配送・設置業務を担当する中で、従来の共有Excelでは、住所や詳しい作業内容、集合場所などを十分に共有できず、毎朝、口頭やメールで個別に確認する手間がありました。この課題を改善するために開発しました。

社員が予定と案件の詳細を登録すると、配送・設置担当者はスケジュール上の案件を選ぶだけで、当日の作業に必要な情報を確認できます。使い慣れたExcelに近い表形式を残し、30分単位で予定の埋まり具合を把握できるようにしました。

入力内容は自動保存され、同じ時間帯への二重登録も防ぎます。開発者本人が要件定義から設計・実装・テスト・運用まで担当しました。
