...
配送・設置の予定と作業に必要な情報を、職場の10人弱で共有するWebアプリである。PCとiPhoneから案件を登録・確認でき、現在は実務で運用している。

## 概要

開発者自身が配送・設置業務を担当する中で、従来の共有Excelでは、住所や詳しい作業内容、集合場所などを十分に共有できず、毎朝、口頭やメールで個別に確認する手間があった。この課題を改善するために開発した。

社員が予定と案件の詳細を登録すると、配送・設置担当者はスケジュール上の案件を選ぶだけで、当日の作業に必要な情報を確認できる。使い慣れたExcelに近い表形式を残し、30分単位で予定の埋まり具合を把握できるようにした。

入力内容は自動保存され、同じ時間帯への二重登録も防ぐ。開発者本人が要件定義から設計・実装・テスト・運用まで担当した。
...
初期MVPと初回導入機能、UI改善、クラウド対応まで完了している。H2、PostgreSQL Testcontainers、Playwright Chromiumを含む自動テストをGitHub Actionsで実行している。

- 受入テストでは社員5人程度が架空データで主要操作を確認し、正式導入を妨げる重大な問題は検出されなかった
- 受入テスト後、10人弱を対象とする正式運用を開始し、実在案件を入力している

開発者自身も、社員から配送・設置の依頼を受ける担当者として日常的に利用している。導入前は、Excelだけでは分からない作業内容や集合情報を毎朝、口頭やメールで確認していた。導入後は案件の詳細画面から確認できるようになり、個別確認の手間が減ったと実感している。
...
Render Free Web ServiceとNeon Free PostgreSQL上で実在案件の入力を開始している。日次PostgreSQLバックアップ、SHA-256検証、隔離復元、障害時手順は架空データで検証済みであり、正式運用中の実施記録は[運用ランブック](docs/operations-runbook.md)に従って管理する。
...
詳しい選定理由と同時実行方式は、[技術選定・検証記録](docs/technical-decisions.md)に記載している。

## 運用環境

正式運用では、Render Free Web Serviceでアプリを実行し、Neon Free PostgreSQLへ案件を保存している。10人弱の利用者へ業務用URLをメールで共有し、共通パスワードを入力して利用する方式である。

業務用URLは実在案件を扱うため、ポートフォリオ閲覧者向けの公開デモには使用しない。READMEの起動手順では、本番DBや秘密情報へ接続せず、架空データで安全に確認できるDockerまたはローカルH2の手順を案内している。

## 起動方法

以下は、第三者がローカル環境で動作を確認するための手順である。
...
主要な設計資料はREADME上部に掲載している。開発履歴を含む全資料は次のとおりである。
...
初期MVP後に、任意年月選択、祝日カレンダー連携、休み設定、案件コピー、固定予定、全面UIリニューアル、クラウド対応を段階的に追加した。画面からは案件の入力、編集、詳細確認に加え、6種類の作業種別、依頼内容、住所、顧客先到着希望時間、同行時の集合場所、出発時間、車両を管理できる。

個人別ログインと権限管理は導入せず、クラウド環境だけ共通パスワードゲートを使用する。案件ステータス管理、地図連携、ルート最適化、業務予定の外部カレンダー連携は将来拡張として扱う。現行Excelの予定は取り込まず、新システムは空の状態から利用を開始する方針である。

利用者は共有URLをブラウザで開いて利用する。PWAなどの正式なインストール機能は設けず、必要な場合はブラウザのブックマークやホーム画面ショートカットを利用する。
