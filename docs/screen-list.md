...
| 表示項目 | 対象年月、月タブ、日付、時間帯、先頭セルの依頼者名、2セル目以降の矢印、案件単位の色 |
| 主な操作 | 月切り替え、セルクリック、案件入力・編集フォームへの遷移 |
| MVP | 最重要 |
...
- 画面下部または上部に月タブを置き、前月・当月・翌月などへ切り替えられるようにする
...
- 1日あたりの案件数は多くても4件程度の想定のため、5色程度の色を用意し、その日の1件目、2件目のように順番で色を割り当てる
- セルクリックは日付選択の入口とし、クリックした時間帯で入力時間を固定しない
...
| 必須項目 | 依頼者名、開始時間、終了時間、作業種別、依頼内容、住所、現場到着希望時間、一人作業 / 同行あり |
| 条件付き必須 | 同行ありの場合、集合場所、出発時間、使用車両 |
| 任意項目 | 出庫要否、備考 |
...
- 確定ボタンは置かない
- 明示的な確定ステータスは持たない
- 入力内容は自動保存または離脱時保存など、確定ボタン不要の方式で反映する
- 保存方式の詳細は実装設計で決める
- 時間重複時は保存せず、注意表示を出す
