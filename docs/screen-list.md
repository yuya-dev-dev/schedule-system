...
個人別ログイン、ユーザー管理、権限差は導入せず、社員も配送・設置担当者も同じ画面を同じ権限で参照・編集する。cloud profileだけは、利用者を識別しない共通パスワード入力画面を入口制限として表示する。
...
| S-005 | 共通パスワード入力画面 | クラウド利用者 | 共有URLへ入る前に共通パスワードを確認する |
...
- 通常案件は5色の固定パレットから、同じ日の開始時間が早い案件順に自動割り当てする
- 入庫、商品管理は通常案件と明確に区別できる専用色を使用する
...
- 今月・翌月の水曜日8:30-10:00には入庫、金曜日8:30-10:00には商品管理を自動表示する。ただし祝日、休み、過去日、既存予定との重複日、個別削除済みの日は除く
...
- 入庫、商品管理は固定予定として自動作成されるほか、通常の入力フォームからも手動登録できる。依頼者名は任意とする
...
- PCとiPhoneのどちらからでもスケジュール一覧の閲覧と案件の入力・編集を行える
- iPhoneではスケジュール一覧、案件入力フォーム、案件詳細を正常に表示できる
- 主要な入力、編集、キャンセル操作をPCとiPhoneで利用できる
...
- 下書きは時間枠を確保しない
- 下書き削除は確認後に物理削除する
- 作業日を過ぎた下書きは起動時と、アプリ稼働中の毎日3時（日本時間）に保守処理が物理削除する。下書き一覧の表示自体はDBを更新しない
...
- 入庫・商品管理の固定予定を個別キャンセルした場合は、その日を再作成対象から除外する
...
| 項目 | 内容 |
| --- | --- |
| 目的 | クラウドURLを知る利用者だけが業務画面へ進める最低限の入口制限 |
| 表示項目 | 共通パスワード、認証失敗、試行制限、ログアウト完了メッセージ |
| 主な操作 | 共通パスワード送信。認証後は月間画面からログアウト |
| 適用環境 | cloud profileのみ |

補足:

- 個人を識別するログインではなく、全員が同じ共通パスワードを使用する
- パスワードはRenderの環境変数で管理し、画面、文書、Gitへ値を記録しない
- 同一送信元で15分以内に5回失敗した場合は15分間ログインを拒否し、認証後は30分の無操作でセッションを失効する
- cloud profileでアクセスゲートが無効な場合はアプリを起動しない
- ログインを含む状態変更POSTはCSRFトークンで保護する

## 初期MVP後の追加画面

フェーズ5では、任意年月選択、祝日除外、休み設定、案件コピー、全面UIリニューアルを追加済みである。F-007とF-008は実装済み画面として扱い、それ以外は必要性を確認して将来拡張とする。
...
| F-008 | 休み設定確認画面 | 対象日の案件件数を確認して休み表示へ置き換える |

フェーズ5Fの全面UIリニューアルは新しい業務機能や画面を追加する工程ではなく、S-001からS-004、F-007、F-008を共通のデザイン規則で再構成する工程として完了済みである。

フェーズ5追加画面の受入条件:

- 任意年月選択は年の数値入力と月の選択を使用し、表示中の年月にかかわらず下部3タブは現在月基準の前月・当月・翌月を表示する
- 祝日列は一覧から除外し、祝日への新規入力・編集画面へは遷移しない
- F-007はコピー元の月を初期表示し、直接日付入力、簡易スケジュール表、年月選択を提供する。同じ日、過去日、祝日、休みは選択できない。コピー先確定時に新規保存を試み、重複時は未保存のコピー値と赤字エラーを持つ入力フォームを表示する
...
```mermaid
flowchart TD
    L["共通パスワード入力<br>cloudのみ"] --> A["スケジュール一覧"]
    A --> B["新規入力フォーム"]
    A --> C["既存案件の入力・編集フォーム"]
    A --> E["下書き一覧"]
    E --> B
