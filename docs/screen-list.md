...
初期MVPではS-001からS-005を対象に、最小限の項目入力、自動保存、一覧反映、祝日列の自動除外、二重確認付きの依頼キャンセル・休み設定、案件コピーまで実際に動作させる。細かなUI調整は、その後の反復開発で追加する。
...
| 主な操作 | 月切り替え、日付の休み設定・解除、空白セルクリックによる新規入力、入力済みセルクリックによる既存案件の確認・編集 |
| MVP | 最重要 |
...
- 外部の日本祝日カレンダーから祝日を取得し、祝日と重なる水曜日・金曜日は自動的に表示対象から除外する
