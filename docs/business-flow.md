...
2. フォーム上部の `別の日程にコピー` を押す
3. コピー先の勤務日を選び、日付以外の入力値が入った新規フォームを開く
4. コピー先の同じ時間帯に既存案件がなければ、新規案件として保存する
5. 時間帯が重なる場合は保存せず、コピー内容を残したまま重複エラーを表示する
6. 必要に応じて開始時間・終了時間を修正して保存する
7. コピー元の案件はそのまま残す
