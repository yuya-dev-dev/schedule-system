...
入庫、商品管理も同じ入力フォームから手動登録し、依頼者名、開始時間、終了時間を入力する。
...
2. 出勤できない水曜日・金曜日は、日付見出しから休み設定を開始する
3. 確認ダイアログに対象日と削除される案件数を表示する
4. 利用者が実行を確定すると、その日の全案件を物理削除する
5. 全時間セルを休み表示へ置き換え、以後の案件入力を禁止する
6. 出勤可能に戻った場合は休みを解除する

祝日は外部の日本祝日カレンダーから取得し、該当する水曜日・金曜日の日付列をシステムが自動的に一覧から除外する。取得に失敗した場合は前回取得値を使用し、前回値もない場合は水曜日・金曜日を通常表示して業務を継続する。
