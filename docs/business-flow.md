...
入庫、商品管理も同じ入力フォームから手動登録でき、依頼者名は任意とする。これとは別に、システムは今月・翌月の水曜日8:30-10:00へ入庫、金曜日8:30-10:00へ商品管理を不足分だけ自動作成する。
...
7. 作業日を過ぎた下書きは、起動時とアプリ稼働中の毎日3時（日本時間）に自動で物理削除する

下書きの状態では時間枠を確保しない。月間一覧のGETは下書きを削除せず、表示データの取得だけを行う。作業日を過ぎて保守処理により自動削除された下書きは復元しない。
...
1. アプリ起動時と、稼働中の毎日3時（日本時間）に祝日キャッシュを確認する
2. 祝日データが利用可能な場合、今月・翌月の固定予定の不足分を確認する
3. 水曜日8:30-10:00へ入庫、金曜日8:30-10:00へ商品管理を作成する
4. 祝日、休み、過去日、既存予定との重複日、利用者が固定予定を個別削除した日は作成しない
5. 休みを解除した場合は、今月・翌月の対象日だけ固定予定を即時再作成する
6. 固定予定作成後、作業日を過ぎた下書きを削除する

固定予定は通常案件と異なる専用色で表示する。Render無料枠の停止中に3時を過ぎた場合は、次回起動時に同じ保守処理を実行する。
...
個人別ログインや権限差は設けない。正式運用では、社員と配送・設置担当者を合わせた10人弱の利用者が、現行の共有Excelと同様に1つのスケジュールを同じ権限で利用する。クラウド環境では共通パスワードゲートだけを入口制限として使用する。
...
初期MVPでは、通知や祝日カレンダー連携がなくても、Excelセルに収まらない情報を案件詳細として管理できる状態を最初の到達点とした。現在はフェーズ5で祝日除外、休み設定、案件コピー、任意年月選択、UIリニューアルを追加済みである。
