...
- 送信元の記録上限に達した場合は最古の記録を破棄し、未追跡の送信元を一括拒否しない。
...
共通パスワード方式に残る個別認可・個別失効・操作主体識別の欠如は、10人弱の現行運用におけるAccepted Riskとする。運用上は長いランダムパスワード、共有先の限定、漏えい時の即時変更で補完し、個人アカウントやRBACは要件が変わった場合の将来改善とする。
...
永続化する新規`DRAFT`は全体で100件を上限とし、PostgreSQLでは上限判定と保存をtransaction-level advisory lockで直列化する。上限到達後も入力条件を満たす`PUBLISHED`案件の新規保存は妨げない。月間画面の下書き一覧も更新日時の新しい100件までに制限し、DB行数と一覧処理が無制限に増える経路を閉じる。
