...
個人別ログイン、ユーザー識別、権限管理、案件の業務ステータス管理は行わない。クラウド配置時の共通パスワードゲートはアプリ利用者の識別ではなく、外側の入口制限として扱う。現行Excelに近い軽い運用を優先し、案件情報を1件単位で保存する。一覧反映可否を管理する技術状態としてのみ `DRAFT` と `PUBLISHED` を持つ。通常の水曜日・金曜日は年月から算出し、休み設定と日本の祝日取得はMVP後に追加する。
