...
MVPでは、ログイン、ユーザー識別、権限管理、案件ステータス管理を行わない。現行Excelに近い軽い運用を優先し、案件情報を1件単位で保存する。通常の水曜日・金曜日は年月から算出し、個人的な休みだけを日付設定として保存する。日本の祝日は外部カレンダーから取得し、取得結果をDBへキャッシュする。
...
erDiagram
    ...
    SCHEDULE_DAY_OFFS o|--o{ SCHEDULE_REQUESTS : controls_date
...
| schedule_day_offs | 個人的に出勤できない水曜・金曜の休みを管理する | 対象 |
| calendar_holidays | 外部カレンダーから取得した日本の祝日をキャッシュする | 対象 |
...
- 外部カレンダーの同期で既存案件の日付が新たに祝日と判定された場合は、該当案件を物理削除し、祝日データの保存と同一トランザクションで処理する。
- 祝日同期で削除した案件の復元用レコードや履歴はMVPでは保持しない。
- 外部カレンダーの具体的な取得元、同期頻度、取得失敗時の動作は後続ヒアリングで確定する。
...
- サーバーのシステム日付を基準に、`work_date` の年月が現在年月より前の場合は過去月として扱う。
- 過去月の一覧と案件詳細は取得できるが、新規作成、更新、物理削除、コピー、休み設定変更を拒否する。
- 画面上の無効化だけに依存せず、Service層でも過去月への変更を検証する。
- 現在月内の経過日は編集可能とする。
- 未来月の登録上限は設けず、上限月数を管理する設定値も持たない。
- 未来日でも、水曜日・金曜日かつ休み・祝日ではないことを検証する。
