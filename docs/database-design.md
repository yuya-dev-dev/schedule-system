...
- `HIDDEN` 設定前に対象日を確認画面へ表示し、利用者の確認後に保存する。
- MVPでは `HIDDEN` を解除する画面操作を用意しないが、将来は例外レコードの削除により再表示できる構造とする。
...
- 過去月の一覧と案件詳細は取得できるが、新規作成、更新、物理削除、コピー、日付設定変更を拒否する。
- 画面上の無効化だけに依存せず、Service層でも過去月への変更を検証する。
- 現在月内の経過日は編集可能とする。
- 未来月の登録上限は設けず、上限月数を管理する設定値も持たない。
- 未来日でも、水曜日・金曜日かつ `DAY_OFF`、`HIDDEN` ではないことを検証する。
