...
個人別ログイン、ユーザー識別、権限管理、案件の業務ステータス管理は行わない。クラウド配置時の共通パスワードゲートはアプリ利用者の識別ではなく、外側の入口制限として扱う。現行Excelに近い軽い運用を優先し、案件情報を1件単位で保存する。一覧反映可否を管理する技術状態としてのみ `DRAFT` と `PUBLISHED` を持つ。通常の水曜日・金曜日は年月から算出し、休み設定と日本の祝日取得はフェーズ5で追加済みである。
...
- `recurring_fixed_request_skips` は利用者が個別削除した固定予定の日付だけを記録し、自動再作成を防ぐ。
...
| schedule_day_offs | 日付単位の休み設定を管理する | フェーズ5で追加 |
| calendar_holidays | 外部カレンダーから取得した日本の祝日をキャッシュする | フェーズ5で追加 |
| recurring_fixed_request_skips | 個別削除した固定予定の再作成を防ぐ | 固定予定機能で追加 |
...
- 入庫、商品管理も `schedule_requests` に保存する。通常フォームから手動登録できるほか、今月・翌月の固定予定を定期保守で自動生成する。
...
- `work_date` が日本時間の現在日付より前である下書きは、起動時とアプリ稼働中の毎日3時にスケジュール保守処理が物理削除する。下書き一覧の取得処理は読み取り専用とする。
...
- 1か月保持の自動削除はcloud profileのアプリ起動時に実行し、削除件数だけをログへ出す。これとは別に、作業日を過ぎた下書きはスケジュール保守が起動時と毎日3時に削除する。住所、顧客名、依頼内容などの業務情報はログへ出さない。
- 自動削除したレコードの復元用レコードや削除履歴は保持しない。正式運用では運用ランブックに従ってバックアップと復元手順を管理する。
...
- PostgreSQLでは既定の `READ COMMITTED` とGiST排他制約を使用し、SQLSTATE `23P01` を時間重複として扱う。同じ排他制約の検査中に発生した `40P01` は自動保存の競合回復対象とする。
...
- 取得に失敗し、保存済みデータもない場合は、祝日除外を行わず水曜日・金曜日を通常表示し、固定予定の自動作成だけを見送る。外部サービスの障害だけを理由に一覧表示や案件入力を停止しない。
- 取得元は政府の公式公開データを優先し、実装前に利用条件とデータ形式を確認する。民間APIやGoogleカレンダーへは依存しない。
- アプリ起動時と、アプリ稼働中の毎日3時（日本時間）にキャッシュ期限を確認し、古い場合だけ自動同期する。一覧表示ごとの外部通信と手動同期操作は行わない。
- CSVはヘッダ、列数、日付、祝日名、重複日付を厳格に検証し、1行でも不正なら同期全体を失敗として既存キャッシュを保持する。
- 祝日に案件を入れる運用はない前提とし、祝日列は一覧から除外する。祝日同期を理由に案件データを自動削除する処理や、`祝日・登録あり` の例外表示は作らない。

## 固定予定追加: recurring_fixed_request_skips

水曜日8:30-10:00の入庫と金曜日8:30-10:00の商品管理は、通常案件と同じ `schedule_requests` の `PUBLISHED` レコードとして保存する。自動生成専用の状態や外部キーは追加しない。

| カラム | 型 | 制約 | 説明 |
| --- | --- | --- | --- |
| work_date | DATE | PK | 利用者が固定予定を個別削除し、再作成を抑止する日付 |
| created_at | TIMESTAMP | NOT NULL | skip記録の作成日時 |

- 自動作成対象は今月と翌月だけとする。
- 祝日、休み、過去日、同じ時間帯に `PUBLISHED` の既存予定がある日は作成しない。
- 固定予定を個別キャンセルした場合だけskipを記録してから案件を削除する。
- 休み設定による一括削除ではskipを記録せず、休み解除時に今月・翌月の対象日を即時補充する。
- 同時実行で固定予定作成が競合した場合も、PostgreSQLのSQLSTATE `23P01` だけを正常な時間競合として扱う。
...
作業種別そのものは任意とする。未入力の場合は通常作業と同じく、依頼者名、開始時間、終了時間がそろえば一覧へ反映し、作業種別だけを理由に不足警告を表示しない。
...
固定予定の個別キャンセル時は `recurring_fixed_request_skips` へ対象日を記録し、次回保守で同じ固定予定を作り直さない。

変更履歴や復元が必要になった場合は、将来拡張でキャンセル履歴や監査ログを検討する。

## 正式運用のバックアップ方針

DBバックアップ機能は初期MVPの完成条件に含めなかった。正式運用では、PostgreSQLの配置先に合わせたバックアップ方法と保管先を使用し、架空データで検証した隔離復元手順を継続する。実施結果は実在案件情報や秘密値を含めず社内の運用記録で管理する。

## 初期サンプルデータ方針

サンプルデータは全て架空情報で作成する。

デモで主要な画面状態を確認できるよう、1日あたり3件程度の案件を基本にし、設置、回収、交換、配達、入庫、商品管理の作業種別を一通り含める。
