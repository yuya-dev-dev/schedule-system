...
- RPO（目標復旧時点）は、営業日ごとの日次バックアップが成功していることを前提に最大1営業日とする。Neonのinstant restoreでより新しい時点へ戻せる場合があっても、長期保持の保証としては扱わない。
- RTO（目標復旧時間）は現時点で未計測とし、保証値を設定しない。月1回の隔離復元では開始から検証完了までの所要時間を運用記録へ残し、復旧手順全体の実測結果が得られた時点で目標値を決定する。
