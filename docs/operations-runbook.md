...
この手順は、Render Free Web ServiceとNeon Free PostgreSQLを使う5人規模の限定利用を対象とする。限定利用は許可済みだが、会社承認済みの保管先、日次担当者、初回本番バックアップと隔離復元記録がそろうまでは正式運用と区別する。共通パスワード、Neon接続情報、バックアップ設定ファイル、バックアップ本体はGit、README、会話、スクリーンショットへ保存しない。
